  ...
  const create = ctx.tools.register(
    defineTool({
      ...
      description:
        ...
        'directory is named after it instead. In a worktree the branch is yours to change: create or switch ' +
      ...
      parameters: {
        branch: {
          type: 'string',
          description: 'Branch to create or check out. Defaults to a generated `worktree/<code>`, cut from `base`.',
        },
        base: {
          type: 'string',
...
function registerRoutes(ctx, settings, repoSummary, archiveDeps) {
  const routes = [
    {
      path: HEALTH_PATH,
      handler: async () => ({
...
export function apply(ctx, config) {
  ...
  const extras = { sessions: undefined, projections: undefined, cache: undefined, persistence: undefined }
  ...
  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => registerRoutes(webCtx, settings, repoSummary, archiveDeps), 'gord-dsh-worktree: routes')
  })
}
