...
/** Bumped with the package version; surfaced by `GET /dsh-worktree/health`. */
...
/**
 * Panel defaults. `defaultParent` empty means "a sibling `<repo>-worktrees`
 * directory"; `adoptWorkspace` registers a created worktree as a DSH workspace
 * so the user can start a session in it without a second step.
 */
export const Config = z.object({
  defaultParent: z.string().default(''),
  adoptWorkspace: z.boolean().default(true),
})

const NS = 'worktree'
/** Read-only metadata endpoint the panel uses to identify the host half. */
const HEALTH_PATH = '/dsh-worktree/health'
const ROUTES_PATH = '/dsh-worktree/api'
 ...
 * @param {{ get: () => { defaultParent: string, adoptWorkspace: boolean } }} settings - resolved panel settings.
    ...
    defineTool({
      ...
      output: {
        ...
        render: (_args, value) => [
          {
            type: 'text',
            text: `Repository ${value.mainRoot} (branch ${value.branch}${value.dirty ? ', uncommitted changes' : ''})\n${worktrees.formatWorktrees(value.worktrees, undefined)}`,
          },
        ],
      },
  ...
  const create = ctx.tools.register(
    defineTool({
      ...
      description:
        ...
        'The new directory is a sibling of the repository by default (`<repo>-worktrees/<branch>`), never inside it. ' +
        'This does not switch the current session into the new worktree — start a session there, or pass its path as workdir to later commands.',
      parameters: {
        branch: {
          type: 'string',
      ...
      output: {
        ...
        render: (_args, value) => [
          {
            type: 'text',
            text: `Created worktree ${value.path} on ${value.createdBranch ? 'new branch' : 'existing branch'} ${value.branch} from ${value.base}.`,
          },
        ],
      },
      async execute(args, exec) {
    ...
    },
    {
      ...
      handler: async (req) => {
        ...
        if (action === 'create') {
          ...
          if (resolved.adoptWorkspace && registry !== undefined) {
            try {
              const entity = await registry.create(result.path)
              workspace = { workspaceId: entity.id ?? entity.workspaceId, path: result.path, title: entity.title }
            } catch (error) {
 ...
 *
 * @param {import('@deepseek-ai/cordis').Context} ctx - plugin context.
 * @param {{ defaultParent: string, adoptWorkspace: boolean }} config - validated composition config.
 */
export function apply(ctx, config) {
  let resolved = { defaultParent: config?.defaultParent ?? '', adoptWorkspace: config?.adoptWorkspace !== false }
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    adopt()
    settingsCtx.effect(() => scope.watch(adopt), 'dsh-worktree: settings observer')
  })
  ...
  ctx.inject(['workspaceRegistry'], (workspaceCtx) => {
    ...
    workspaceCtx.effect(() => () => {
      registry = undefined
    }, 'dsh-worktree: workspace registry release')
  })

  ctx.inject(['webServer'], (webCtx) => {
    webCtx.effect(() => registerRoutes(webCtx, settings, repoSummary, () => registry), 'dsh-worktree: routes')
  })
}
