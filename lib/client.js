  ...
  factory: (require) => {
    ...
    var DICT = {
      zh: {
        ...
        'chip.loading': '读取中…',
        'chip.title': '工作位置',
        'chip.desc': '在这个仓库的工作树中打开一个会话；会话的目录在创建时固定，所以选择会新开一个会话。',
        'chip.new': '新建工作树',
        'chip.newDesc': '从某个分支或远程分支开一个新的检出目录',
        'chip.pick': '打开',
        'chip.pickHint': '在该工作树中打开一个会话',
        'chip.opening': '打开中…',
        'chip.openFailed': '打开会话失败：{message}',
        'chip.empty': '该仓库还没有关联工作树',
      ...
      en: {
        ...
        'chip.label': 'Worktree',
        'chip.none': 'Not in a worktree',
        'chip.this': 'current worktree',
        'chip.loading': 'Reading…',
        'chip.title': 'Working location',
        'chip.desc': 'Open a session inside one of this repository\u2019s worktrees. A session\u2019s directory is fixed when it is created, so picking one opens a new session.',
        'chip.new': 'New worktree',
        'chip.newDesc': 'Check out a new branch or a remote branch into its own directory',
        'chip.pick': 'Open',
        'chip.pickHint': 'Open a session in this worktree',
        'chip.opening': 'Opening…',
        'chip.openFailed': 'Could not open the session: {message}',
        'chip.empty': 'This repository has no linked worktrees yet',
    ...
    /**
     * Composer chip: the working-location picker that sits in the input row.
     *
     * The core composer's own selectors occupy `single` slots, so this plugin
     * cannot stand beside them; `conversation.input.left` is a `list` slot in
     * the same row and is where this lands instead.
     *
     * A DSH session's directory is fixed at creation, so picking a worktree
     * cannot retarget the running session — it opens a session there, which is
     * the same model the core workspace picker uses.
     *
    ...
    function WorktreeChip(props) {
      ...
      var label = t('chip.label');
      var sub = t('chip.none');
      if (snap.phase === 'loading') sub = t('chip.loading');
      if (snap.phase === 'ready') {
        var list = snap.data.worktrees || [];
        var current = null;
        for (var i = 0; i < list.length; i += 1) {
          if (list[i].current) current = list[i];
        }
        sub = current === null ? t('chip.this') : (current.branch || t('chip.this'));
      }
      ...
      if (open) {
        ...
        if (snap.phase === 'norepo' || snap.phase === 'error') {
          items.push(
            React.createElement('div', { key: 'msg', style: S.desc },
              snap.phase === 'norepo' ? t('chip.notRepo') : messageOf(t, snap.payload)),
          );
        }
        if (snap.phase === 'ready') {
          var trees = snap.data.worktrees || [];
          if (trees.length === 0) items.push(React.createElement('div', { key: 'empty', style: S.desc }, t('chip.empty')));
          trees.forEach(function (entry) {
            items.push(
              React.createElement(
                'div',
                { key: entry.path, style: S.rowBetween },
                React.createElement(
                  'div',
                  { style: S.field },
                  React.createElement('span', { style: S.title }, entry.branch || '(detached)'),
                  React.createElement('span', { style: S.desc }, entry.path),
                ),
                React.createElement(
                  Button,
                  {
                    ...
                    onClick: function () {
                      setNote(null);
                      openIn(entry.path, true);
                    },
                  },
                  entry.current ? t('badge.current') : t('chip.pick'),
                ),
              ),
            );
          });
          // The branch list is what makes a remote-only branch reachable at
          // all: naming it here is how the user picks up someone else's work
          // instead of silently forking a new branch of the same name.
          var branches = snap.data.branches || [];
          if (branches.length > 0) {
            items.push(
              React.createElement('div', { key: 'branches', style: S.desc }, branches.join('  ·  ')),
            );
          }
        }
        if (form.open) {
          items.push(
            React.createElement(
              'div',
              { key: 'newform', style: S.field },
              React.createElement(Field, {
                label: t('create.branch'),
                hint: t('create.branchHint'),
                placeholder: t('create.branch'),
                value: form.branch,
                onChange: function (value) { setForm(Object.assign({}, form, { branch: value })); },
              }),
              React.createElement(Field, {
              ...
              React.createElement(
                'div',
                { style: S.actions },
                React.createElement(Button, { disabled: busy, onClick: submitCreate }, busy ? t('create.running') : t('create.submit')),
              ),
            ),
          );
        } else {
          items.push(
            React.createElement(
              'div',
              { key: 'new', style: S.field },
              React.createElement(
                Button,
                { disabled: busy, onClick: function () { setForm({ open: true, branch: '', base: '' }); } },
                t('chip.new'),
              ),
              React.createElement('span', { style: S.desc }, t('chip.newDesc')),
            ),
          );
        }
        if (note) {
        ...
        items.push(React.createElement('div', { key: 'hint', style: S.desc }, t('chip.desc')));
        body = React.createElement('div', { style: S.card }, items);
      }
    ...
    function apply(ctx) {
      ...
      // The composer's own selectors sit in `single` slots, so a second one
      // cannot stand beside them; `conversation.input.left` is a `list` slot in
      // the same row and takes occupants. Registering from inside
      // `ctx.inject` rather than from `apply` keeps the chip's dependency on
      // `sessions` optional: if the session controller is absent the chip is
      // simply not contributed, instead of the whole plugin failing to apply.
      // `slots` is named here even though `apply` already injects it: every
      // core plugin that registers from a child scope lists each service its
      // body reads, and reading an un-injected service as a property throws —
      // the same failure that once blanked the whole Settings page.
      ctx.inject(['slots', 'sessions'], function (scope) {
        scope.slots.inject('conversation.input.left', function () {
          return scope.slots.register({
            ...
            return React.createElement(WorktreeChip, { ctx: scope, t: localeT(scope) });
          });
        });
      });
    }
