  ...
  factory: (require) => {
    ...
    var DICT = {
      zh: {
        ...
        'archive.title': '归档的会话',
        'archive.desc': '本机所有已归档的会话。归档只是把它们从侧栏藏起来——日志一直都在，取消归档就回来了。',
        'archive.count': '共 {n} 条',
        ...
        'archive.archivedUnknown': '归档时间未记录',
        'archive.size': '{size}',
      ...
      en: {
        ...
        'archive.desc': 'Every session archived on this machine. Archiving only hides one from the sidebar — its log is kept, and unarchiving brings it back.',
        ...
        'archive.archivedUnknown': 'archive time not recorded',
        'archive.size': '{size}',
    ...
    function ArchiveSection(props) {
      ...
      var rows = records.map(function (row) {
        ...
        if (row.createdAt) facts.push(React.createElement('span', { key: 'made' }, t('archive.createdAt', { time: formatTime(row.createdAt) })));
        facts.push(React.createElement(
          'span',
          { key: 'at' },
          row.archivedAt
            ? t('archive.archivedAt', { time: formatTime(row.archivedAt) })
            : t('archive.archivedUnknown'),
        ));
        if (row.sizeBytes) facts.push(React.createElement('span', { key: 'size' }, t('archive.size', { size: formatBytes(row.sizeBytes) })));

        return React.createElement(
          'div',
          { key: row.id, className: 'gord-dsh-worktree-archive-row', style: S.item, 'data-gord-archive-row': row.id },
          React.createElement(
            'div',
            { style: S.itemText },
            React.createElement(
            ...
            React.createElement('div', { style: S.meta }, facts),
          ),
          React.createElement(
            Button,
            {
