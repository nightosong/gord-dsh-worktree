  ...
  factory: (require) => {
    ...
    var DICT = {
      ...
      en: {
        ...
        'chip.empty': 'This project has no other worktrees yet',
    ...
    var S = {
      ...
      heroRow: { display: 'flex', alignItems: 'center', gap: '10px', minWidth: '0', flexWrap: 'wrap' },
      heroLabel: { color: 'var(--dsw-alias-label-secondary, #6b7280)', fontSize: '13px', lineHeight: '20px', flex: 'none' },
      heroSelect: {
        boxSizing: 'border-box',
        display: 'inline-flex',
        alignItems: 'center',
      ...
      heroValue: { overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', minWidth: '0' },
      heroCaret: { color: 'var(--dsw-alias-label-tertiary, #8b93a1)', fontSize: '10px', flex: 'none' },
      heroMenu: { flexBasis: '100%', minWidth: '0' },
    ...
    var HOVER_CSS =
      ...
      '.gord-dsh-worktree-select:hover{border-color:var(--dsw-alias-brand-primary, #4176e6)}' +
      '.gord-dsh-worktree-select-open{border-color:var(--dsw-alias-brand-primary, #4176e6)}' +
    ...
    function WorktreeChip(props) {
      ...
      // A dropdown, matching the workspace and preset controls it sits with:
      // a labelled row that reads like the Hero's own selectors rather than a
      // pill tucked into the composer tool row.
      return React.createElement(
        'div',
        { style: S.heroRow },
        React.createElement('span', { style: S.heroLabel }, label),
        React.createElement(
          'button',
          {
            type: 'button',
            className: 'gord-dsh-worktree-select' + (open ? ' gord-dsh-worktree-select-open' : ''),
            style: S.heroSelect,
            onClick: function () { setOpen(!open); },
            title: t('chip.pickHint'),
            'aria-expanded': open,
          },
          React.createElement('span', { style: S.heroValue }, sub),
          React.createElement('span', { style: S.heroCaret }, open ? '\u25b4' : '\u25be'),
        ),
        open ? React.createElement('div', { style: S.heroMenu }, body) : null,
      );
    }
