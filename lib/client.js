...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var S = {
      ...
      // Same metrics as the neighbouring selectors so the row stays one line.
      heroSelect: {
        boxSizing: 'border-box',
        display: 'inline-flex',
        alignItems: 'center',
    ...
    function WorktreeChip(props) {
      ...
      var choiceStore = React.useState({ kind: 'local' });
      var choice = choiceStore[0];
      var setChoice = choiceStore[1];
      ...
      if (choice.kind === 'worktree') value = choice.branch || choice.path;
      ...
      if (open) {
        ...
        // Always offered, and first: the project's own directory.
        items.push(option('local', t('chip.local'), snap.phase === 'ready' ? snap.dir : '', choice.kind === 'local', function () {
          setChoice({ kind: 'local' });
          setOpen(false);
        }));
        if (snap.phase === 'ready') {
          (snap.data.worktrees || []).forEach(function (entry) {
            // The main worktree *is* the local directory the row above already
            // offers, so listing it again would only duplicate the choice.
            if (entry.current) return;
            items.push(option(entry.path, entry.branch || '(detached)', entry.path,
              choice.kind === 'worktree' && choice.path === entry.path, function () {
                setChoice({ kind: 'worktree', path: entry.path, branch: entry.branch });
                setOpen(false);
      ...
      var control = React.createElement(
        'div',
        { style: S.anchor, 'data-gord-worktree': 'trigger' },
        React.createElement(
          'button',
          {
            type: 'button',
            ...
            // The neighbouring selectors show a bare value (the workspace chip
            // reads "repo", the preset "标准模式"), so naming this one in a
            // prefix would make it the odd control out. It is named here and
            // labelled in the menu instead.
            title: t('chip.label') + ' · ' + t('chip.pickHint'),
            'aria-label': t('chip.label'),
            'aria-haspopup': 'listbox',
            'aria-expanded': open,
          },
          React.createElement('span', { style: S.heroValue }, value),
          React.createElement('span', { style: S.heroCaret }, open ? '\u25b4' : '\u25be'),
        ),
        open ? React.createElement('div', { style: S.menu, role: 'listbox' }, items) : null,
      );
