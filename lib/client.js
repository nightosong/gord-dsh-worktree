  ...
  factory: (require) => {
    ...
    var DICT = {
      zh: {
        ...
        'create.branch': '分支名',
        'create.branchHint': '留空则自动生成 worktree/<随机码>',
        'create.base': '起点',
        'create.baseHint': '新分支从哪开始；默认当前分支',
        'create.path': '目录',
        'create.pathHint': '留空则放在 ~/.dsh/worktree/ 下',
        'create.submit': '创建',
        'create.running': '创建中…',
        ...
        'archive.updatedAt': '更新于 {time}',
        'archive.createdAt': '创建于 {time}',
        'archive.size': '{size}',
        ...
        'archive.skipped': '另有 {n} 个仍在使用中，已跳过',
      ...
      en: {
        ...
        'create.branchHint': 'Empty generates worktree/<code>',
        'create.base': 'Base',
        'create.baseHint': 'Where the new branch starts; defaults to the current branch',
        'create.path': 'Directory',
        'create.pathHint': 'Empty uses ~/.dsh/worktree/',
        ...
        'archive.desc': 'Every session archived on this machine.',
        ...
        'archive.updatedAt': 'updated {time}',
        'archive.createdAt': 'created {time}',
        'archive.size': '{size}',
        ...
        'archive.skipped': '{n} more are still in use and were skipped',
    ...
    /** Record `mainRoot` as the project of every worktree under it. */
    function publishParents(data) {
      try {
        var main = data.mainRoot;
        if (typeof main !== 'string' || main === '') return;
        var map = JSON.parse(window.localStorage.getItem(PARENTS_KEY) || '{}');
        var changed = false;
        (data.worktrees || []).forEach(function (entry) {
          if (!entry || typeof entry.path !== 'string') return;
          if (entry.path === main) return;
          if (map[entry.path] === main) return;
          map[entry.path] = main;
          changed = true;
        });
    ...
    function ArchiveSection(props) {
      ...
      var rows = records.map(function (row) {
        ...
        var facts = [];
        if (row.cwd) facts.push(React.createElement('span', { key: 'cwd', style: S.mono, title: row.cwd }, shortPath(row.cwd)));
        // One time, not three. The last write is the honest answer to "when was
        // this last used", and the archive time is the plugin's own bookkeeping
        // rather than a fact about the session, so it is not shown at all: it
        // stays in the record file and keeps ordering the list newest-first.
        if (row.updatedAt) facts.push(React.createElement('span', { key: 'at' }, t('archive.updatedAt', { time: formatTime(row.updatedAt) })));
        else if (row.createdAt) facts.push(React.createElement('span', { key: 'at' }, t('archive.createdAt', { time: formatTime(row.createdAt) })));
        if (row.sizeBytes) facts.push(React.createElement('span', { key: 'size' }, t('archive.size', { size: formatBytes(row.sizeBytes) })));
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              React.createElement(
                'div',
                {
                  style: { color: 'var(--dsw-alias-label-primary, inherit)', fontSize: '13px', lineHeight: '20px', wordBreak: 'break-word' },
                  // The title is the label; the id is what the host can always
                  // answer, so it stays reachable when a title exists and is the
                  // label itself when one does not.
                  title: row.title ? row.id : undefined,
                },
                label,
              ),
              pendingDelete === row.id ? null : React.createElement('div', { style: S.meta }, facts),
            ),
            React.createElement(
              'div',
              { style: S.actions },
      ...
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement(
            ...
            React.createElement('div', { style: S.desc }, t('archive.desc')),
          ),
          React.createElement(
            'div',
            { style: S.actions },
    ...
    function WorktreeChip(props) {
      ...
      function startNewWorktree() {
        ...
        // No branch and no base: the host cuts `worktree/<code>` from the
        // current branch and names the directory with the same code, so this
        // needs no input and cannot collide with an earlier one.
        api('create', { dir: dir, branch: '', base: '' }).then(function (data) {
    ...
    /**
     * Glyph for the tab and its guide entry. `@deepseek-ai/dsh-client-ui-primitives`
     * is a virtual module, so a name it does not export is `undefined` and would
     * throw where React renders it; the branch icon this plugin already uses is
     * the fallback.
     */
    var DIFF_ICON = primitives.IconEditOutline16 || primitives.IconBranchOutline16;
