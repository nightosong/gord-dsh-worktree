  ...
  factory: (require) => {
    ...
    var DICT = {
      zh: {
        ...
        'create.branchPlaceholder': '自动生成',
        'create.baseCurrent': '当前分支（{branch}）',
        'create.base': '起点',
      ...
      en: {
        ...
        'create.branchPlaceholder': 'Generated',
    ...
    /**
     * The same labelled row as `Field`, but the value is chosen from a list.
     *
     * A native `<select>` rather than a custom listbox: the base is one choice
     * among many branches, the browser already gives it keyboard navigation and
     * a scrollable popup for long branch lists, and the popup is drawn outside
     * the page so it cannot fight the popover's own outside-click dismissal.
     */
    function Select(props) {
      return React.createElement(
        ...
        React.createElement(
          'select',
          {
            className: 'gord-dsh-worktree-choice',
            style: Object.assign({}, S.input, S.choice),
            value: props.value,
            disabled: props.disabled === true,
            onChange: function (event) { props.onChange(event.target.value); },
          },
          (props.options || []).map(function (entry) {
            return React.createElement('option', { key: entry.value, value: entry.value }, entry.label);
          }),
        ),
        props.hint ? React.createElement('span', { style: S.desc }, props.hint) : null,
      );
    ...
    function WorktreeChip(props) {
      ...
      /**
       * Create a worktree and report where it landed.
       *
       * Deliberately nothing else. An earlier version registered the new
       * directory as a workspace and opened a session rooted in it, which
       * detached the work from the project the user was in: a new sidebar
       * entry appeared and the conversation moved out from under them. The
       * session stays where it is — a worktree is a second checkout of the
       * same project, not a second project — and the created path is reported
       * so it can be handed to the agent as a working directory.
       */
      function submitCreate() {
        setBusy(true);
        setNote(null);
      ...
      /**
       * The bases offered for a new branch: the current branch first, then the
       * local ones, then the remote-tracking ones that have no local twin.
       *
       * The current branch is the default and is listed explicitly rather than
       * left implicit, because "start from where I am" is a real choice the
       * user should see they are making — and because an empty select would
       * read as nothing being selected.
       */
      function baseOptions() {
        var ready = snap.phase === 'ready' ? snap.data : null;
        if (ready === null) return [{ value: '', label: t('create.baseCurrent', { branch: '' }) }];
        var local = ready.localBranches || [];
        var options = [{ value: '', label: t('create.baseCurrent', { branch: ready.branch }) }];
        local.forEach(function (name) { options.push({ value: name, label: name }); });
        (ready.branches || []).forEach(function (name) {
          if (local.indexOf(name) !== -1) return;
          ...
          items.push(
            React.createElement('div', { key: 'newform', style: S.formBlock },
              React.createElement(Field, {
                ...
                placeholder: t('create.branchPlaceholder'), value: form.branch,
                onChange: function (v) { setForm(Object.assign({}, form, { branch: v })); },
              }),
              React.createElement(Select, {
                label: t('create.base'), hint: t('create.baseHint'),
                value: form.base, options: baseOptions(), disabled: busy,
