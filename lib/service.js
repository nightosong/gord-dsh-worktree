/**
 * dsh-worktree — worktree operations, the single implementation behind the
...
export async function createWorktree(input, signal) {
  ...
  const base = typeof input.base === 'string' && input.base.trim() !== '' ? input.base.trim() : info.branch
  ...
  let branch
  try {
    branch = await assertBranchName(info.root, input.branch ?? defaultBranchName(base), signal)
  } catch (error) {
  ...
  const target = input.path !== undefined && String(input.path).trim() !== ''
    ...
    : join(parent, slugifyBranch(branch))
  ...
  if (list.some((entry) => entry.path === absoluteTarget && !existsSync(entry.path))) {
    return {
      ok: false,
      error: 'stale-record',
      path: absoluteTarget,
